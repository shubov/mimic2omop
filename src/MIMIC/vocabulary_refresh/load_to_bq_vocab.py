...
def read_params():
    ...
    params = {
        "steps": [
            "mandatory: indicate '-a' for 'athena', '-m' for (custom) 'mapping' or both"
        ],
    ...
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "ampc:", ["athena", "mapping", "config="]
        )
        if len(opts) == 0:
...
def load_table(table, gs_path, field_delimiter, quote, config):
    ...
    table_path = bq_table.format(  # project=config['bq_target_project'],
        dataset=config["variables"]["@bq_target_dataset"],
        prefix=config["bq_athena_temp_table_prefix"],
        table=table,
    )
...
def main():
    ...
    for step in params["steps"]:
        ...
        if step == "athena":
            for table in config["vocabulary_tables"]:
                gs_path = gs_file_path.format(
                    gs_path=config["gs_athena_csv_path"],
                    table=table.upper(),
                    ext=".csv",
                )
                ...
                rc = load_table(
                    table,
                    gs_path,
                    config["athena_csv_delimiter"],
                    config["athena_csv_quote"],
                    config,
                )
                if rc != 0:
                    rca += 1
        ...
        if step == "mapping":
            ...
            rc = load_table(
                config["custom_mapping_table"],
                gs_path,
                config["mapping_csv_delimiter"],
                config["mapping_csv_quote"],
                config,
            )
            if rc != 0:
                rcm += 1
